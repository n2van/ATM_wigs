...
with gr.Blocks(theme=theme, title="ATMwigs - Try-on Wigs") as demo:
    ...
    origin_image, destination_image = [], []

    # Display all face inputs in a grid layout
    with gr.Row():
        with gr.Column():
            gr.Markdown("### Faces to replace")
            for i in range(num_faces):
                origin = gr.Image(label=f"Face #{i+1} to replace")
                origin_image.append(origin)
        
        with gr.Column():
            gr.Markdown("### Destination faces")
            for i in range(num_faces):
                destination = gr.Image(label=f"Destination face #{i+1}")
                destination_image.append(destination)

    # Empty list for the function parameters but not used in UI
    thresholds_image = [0.0] * num_faces
